...
def _combine_user_and_demo_threads(user_threads: List[Dict], demo_threads: Optional[List[Dict]] = None) -> List[Dict]:
    """Keep user tasks first, then append demo threads in the configured display order."""
    resolved_demo = demo_threads if demo_threads is not None else _load_demo_threads()
    demo_ids = {thread.get("thread_id") for thread in resolved_demo if thread.get("thread_id")}
    ...
    for demo in reversed(resolved_demo):
        tid = demo.get("thread_id")
        if not tid:
            continue
        merged = dict(persisted_demo_meta.get(tid, {}))
        merged.update(demo)
        merged["is_demo"] = True
...
def _drain_pending_stream_events(state: UIState, thread_id: Optional[str]) -> bool:
    """Replay buffered stream events for a thread that was inactive."""
    if not thread_id:
        return False
...
def _auth_guard(
    ...
    *,
    ...
    allow_demo_view: bool = False,
) -> Optional[str]:
    """Block unauthenticated actions while optionally allowing read-only demo access."""
    if allow_demo_view and thread_id and _is_demo_thread(state, thread_id):
        return None
...
def _auth_status_text(state: UIState) -> str:
    if not state.is_authenticated:
        return "🔒 Log in to create a new task. Demo case studies are available below."
    if not state.is_verified:
...
def _guard_and_warn(
    ...
    *,
    ...
    message = _auth_guard(state, thread_id=thread_id, allow_demo_view=allow_demo_view)
    if message:
        gr.Warning(message)
    return message
...
def _new_task_button_update(state: Optional[UIState]):
    is_ready = bool(state and state.is_authenticated and state.is_verified)
...
def _conversation_panel_markup(state: UIState) -> str:
    ...
    if not state.thread_ids:
        empty_text = (
            ...
            else "Browse demo case studies or log in to create your own task."
        )
...
async def _sync_user_threads(state: UIState, ensure_one: bool = True) -> None:
    ...
    if not state.user_id:
        state.thread_ids = _combine_user_and_demo_threads([], demo_threads)
        state.thread_files = {}
        ...
        for tid in list(state.pending_stream_events):
            if tid not in valid_ids:
                state.pending_stream_events.pop(tid, None)
...
async def _activate_thread(thread_id: Optional[str], state: UIState):
    if _guard_and_warn(state, thread_id=thread_id, allow_demo_view=True):
        return (
            state,
            _conversation_panel_update(state),
            list(state.messages),
...
async def _delete_thread(thread_id: Optional[str], state: UIState):
    ...
    state.stale_threads.discard(thread_id)
    valid_ids = {t["thread_id"] for t in state.thread_ids}
    if state.current_thread_id == thread_id or state.current_thread_id not in valid_ids:
...
async def on_conversation_action(action_payload: str, state: UIState):
    ...
    allow_demo_view = action_type == "activate"
    if _guard_and_warn(state, thread_id=thread_id, allow_demo_view=allow_demo_view):
        return (
            state,
            _conversation_panel_update(state),
            list(state.messages),
...
async def _run_user_message_internal(prompt: str, state: UIState, *, approve_signal: Optional[str] = None):
    ...
    try:
        ...
        try:
            while stream_task:
                ...
                if is_active_thread and not ui_attached:
                    ...
                    updated = _drain_pending_stream_events(state, watch_thread_id)
                    if updated:
                        yield (
                            state,
                            list(state.messages),
                ...
                if poll_task and poll_task in done:
                    ...
                    refreshed = (
                        await _refresh_thread_files_for(state, watch_thread_id) if ui_attached else False
                    )
                    if refreshed and ui_attached:
                        yield (
                            state,
                            list(state.messages),
                            gr.update(value=""),
                ...
                if stream_task in done:
                    ...
                    if additions:
                        ...
                        if (
                            event_type == "complete"
                            and watch_thread_id
                            and await _refresh_thread_files_for(state, watch_thread_id)
                        ):
                            yield (
                                state,
                                list(state.messages),
        ...
        if not is_active_thread:
            if watch_thread_id:
                state.stale_threads.add(watch_thread_id)
        elif ui_attached and await _refresh_thread_files_for(state, watch_thread_id):
            yield (
                state,
                list(state.messages),
                gr.update(value=""),

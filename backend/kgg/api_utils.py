...
def getDrugCount(disease_id):
    ...
    Returns:
    ----------
    count (int): The count of drugs associated with the disease.
    '''
...
def GetDiseaseAssociatedDrugs(disease_id,CT_phase):
    ...
    Returns:
    ----------
    df (Pandas DataFrame): DataFrame containing drug information.
    '''
...
def GetDiseaseSNPs(disease_id):
    ...
    Returns:
    ----------
    snps_df (Pandas DataFrame): DataFrame containing SNP information.
    '''
...
def ExtractFromUniProt(uniprot_id) -> dict:
    ...
    Parameters:
    ---------
    uniprot_id (list): UniProt accessions to fetch.

    Returns:
    ----------
    Uniprot_Dict (dict): each accession mapped to its OMIM disease, Reactome pathway, biological process, molecular function and gene symbol.
    '''
...
def RetMech(chemblIds) -> dict:
    ...
    Parameters:
    ---------
    chemblIds (list): ChEMBL molecule identifiers to look up.

    Returns:
    ----------
    named_mechList (dict): each identifier mapped to its mechanisms of action and target proteins, omitting the drugs ChEMBL has none for.
    '''
...
def RetAct(chemblIds) -> dict:
    ...
    Returns:
    ----------
    named_ActList (dict): each identifier mapped to its assays, with organism, target type and pChEMBL value.
    '''
...
def Ret_chembl_protein(sourceList) -> list:
    ...
    Parameters:
    ---------
    sourceList (dict): the assay or mechanism records to scan for targets.

    Returns:
    ----------
    chembl_protein_ids (list): the ChEMBL identifiers among them that are proteins rather than compounds.
    '''
...
def chembl2uniprot(chemblIDs) -> dict:
    ...
    Parameters:
    ---------
    chemblIDs (list): ChEMBL target identifiers to convert.

    Returns:
    ----------
    chem2gene2path (dict): each target mapped to its UniProt accession and the Reactome pathways it belongs to.
    '''
...
def chembl2gene2path(
    ...
    Parameters:
    ---------
    chem2geneList (dict): ChEMBL target to gene-symbol and pathway mapping, from `chembl2uniprot`.
    ActList (dict): the assay records whose protein nodes need the gene symbol.

    Returns:
    ----------
    ActList (dict): the same records with each ChEMBL protein node annotated with its gene symbol.
    '''
...
def chem2moa_rel(
    ...
    Parameters:
    ---------
    named_mechList (dict): drug to mechanism-of-action records, from `RetMech`.
    org (str): organism to keep targets for.
    otp_prots (list): the disease-associated proteins from Open Targets, used to filter targets.
    graph (BELGraph): the graph to add to.

    Returns:
    ----------
    graph (BELGraph): the same graph with drug-to-target mechanism edges added.
    '''
...
def chem2act_rel(
    ...
    Parameters:
    ---------
    named_ActList (dict): drug to assay records, from `RetAct`.
    org (str): organism to keep assays for.
    otp_prots (list): the disease-associated proteins from Open Targets, used to filter targets.
    graph (BELGraph): the graph to add to.

    Returns:
    ----------
    graph (BELGraph): the same graph with bioassay edges added.
    '''
...
def gene2path_rel(
    ...
    Parameters:
    ---------
    named_chem2geneList (dict): target to gene-symbol and pathway mapping.
    org (str): organism to keep proteins for.
    otp_prots (list): the disease-associated proteins from Open Targets, used to filter proteins.
    graph (BELGraph): the graph to add to.

    Returns:
    ----------
    graph (BELGraph): the same graph with protein-to-Reactome-pathway edges added.
    '''
...
def chembl2adverseEffect_rel(
    ...
    '''Method to add drug side-effect edges to the KG.

    Parameters:
    ---------
    chembl_adveff_df (Pandas DataFrame): drug identifiers paired with reported side effects, from Open Targets.
    graph (BELGraph): the graph to add to.

    Returns:
    ----------
    graph (BELGraph): the same graph with `hasSideEffect` edges added.
    '''
...
def uniprot_rel(
    ...
    Parameters:
    ---------
    named_uprotList (dict): UniProt records, from `ExtractFromUniProt`.
    org (str): organism to keep proteins for.
    graph (BELGraph): the graph to add to.

    Returns:
    ----------
    graph (BELGraph): the same graph with UniProt disease, pathway and function edges added.
    '''
...
def protein_annotation_disease_score(graph, dis2prot_df):
    ...
    Parameters:
    ---------
    graph (BELGraph): the graph whose HGNC protein nodes to annotate.
    dis2prot_df (Pandas DataFrame): disease-to-protein association scores from Open Targets.

    Returns:
    ----------
    graph (BELGraph): the same graph with each protein node carrying its association score.
    '''
...
def searchDisease(keyword,logger=None):
    ...
    Parameters:
    ---------
    keyword (str): the disease name to search for.
    logger (Logger): where to report progress and partial results, or None.

    Returns:
    ----------
    api_response (Pandas DataFrame): the candidate disease identifiers Open Targets returned, best match first.
    '''
...
def createKG(disease_id: str, clinical_trial_phase: int, protein_threshold: float, logger=None):
    ...
    Returns:
    ----------
    kg (dict): A dictionary containing the Knowledge Graph data.
    '''
